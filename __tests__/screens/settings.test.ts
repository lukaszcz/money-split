...
describe('Settings Screen', () => {
  ...
  describe('App Information', () => {
    ...
    it('should display app description', () => {
      const description = 'Track shared expenses and debts with friends.';

      expect(description).toBeTruthy();
      expect(description).toContain('expenses');
    });
  });
  ...
  describe('State Management', () => {
    ...
    it('should preserve original name during edit', () => {
      const originalName = 'John Doe';
      let tempName = originalName;

      tempName = 'Jane Smith';
      expect(tempName).not.toBe(originalName);

      // Cancel restores original
      tempName = originalName;
      expect(tempName).toBe(originalName);
    });
  });
});
